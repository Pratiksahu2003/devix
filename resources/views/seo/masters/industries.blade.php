        ...
        <section class="rounded-xl border border-border-subtle bg-white p-4 shadow-sm mb-4">
            ...
            <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($industries as $industry)
                    <a href="{{ seo_url('industry', $service['slug'], $industry['slug']) }}" class="rounded-lg border border-border-subtle p-3 hover:border-brand-lens-blue transition group">
                        <h3 class="text-xs font-semibold text-text-main group-hover:text-brand-lens-blue">{{ $service['name'] }} for {{ $industry['name'] }}</h3>
                        <p class="mt-1 text-[10px] text-text-muted line-clamp-2">{{ $industry['description'] }}</p>
                    </a>
                @endforeach
            </div>
        </section>
