...
@section('page_subtitle', 'Tweak and update your blog post.')
...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="/blog/{{ $post->slug }}" target="_blank" class="inline-flex items-center text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
        Read Live Blog &rarr;
    </a>
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
            ...
            <div class="space-y-5">
                ...
                <div x-data="{ newTag: '', tags: {{ old('tags') ? old('tags') : json_encode($post->tags ?? []) }} }">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="15" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-colors">{{ old('content', $post->content) }}</textarea>
    ...
    <div class="lg:col-span-1 space-y-6 lg:sticky lg:top-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <label class="flex items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors mb-6">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $post->is_published) ? 'checked' : '' }} class="w-5 h-5 text-emerald-600 rounded bg-white border-slate-300 focus:ring-emerald-500 focus:ring-2">
                <div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            @if($post->cover_image)
            <div class="mb-4">
                <img src="{{ asset('storage/' . $post->cover_image) }}" class="w-full h-32 object-cover rounded-xl border border-slate-200 bg-slate-50">
            </div>
            @endif

            <div class="space-y-4">
                <div>
                    <label for="cover_image" class="block text-sm font-semibold text-slate-700 mb-1.5">Replace Cover Image (Banner)</label>
                ...
                <div>
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $post->video_url) }}" class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-colors text-sm" placeholder="https://youtube.com/...">
