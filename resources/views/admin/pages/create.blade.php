...
@section('page_title', 'New SEO Page')
@section('page_subtitle', 'Draft a new top-level landing page.')
...
<form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start" x-data="{ 
    title: '{{ old('title') }}', 
    slug: '{{ old('slug') }}', 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <label class="flex items-center gap-1.5 text-xs text-slate-500 cursor-pointer hover:text-red-600 transition-colors" title="Instruct search engines not to index this specific page.">
                    <input type="checkbox" name="meta_robots_noindex" value="1" {{ old('meta_robots_noindex') ? 'checked' : '' }} class="w-4 h-4 text-red-600 border-slate-300 rounded focus:ring-red-500">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-colors" placeholder="Overwrites the <title> tag.">
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-colors" placeholder="video, production, edit">
                    </div>
                    <div>
                        <label for="canonical_url" class="block text-sm font-semibold text-slate-700 mb-1.5">Canonical URL</label>
                        <input type="url" name="canonical_url" id="canonical_url" value="{{ old('canonical_url') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none transition-colors" placeholder="https://external-site.com/origin">
                    </div>
                </div>
            </div>
        </div>
