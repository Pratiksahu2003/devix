...
<form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start" x-data="{ 
    ...
    <div class="lg:col-span-1 space-y-6 lg:sticky lg:top-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-4">
                <div>
                    <label for="cover_image" class="block text-sm font-semibold text-slate-700 mb-1.5">Cover Image Banner <span class="text-red-500">*</span></label>
                    <input type="file" name="cover_image" id="cover_image" accept="image/*" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-colors" required>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    /* ---- Inline Base64 image upload adapter (no server endpoint required) ---- */
    ...
    ClassicEditor
        ...
        .then(editor => {
            ...
            /* ---- Mark error state on editor wrapper ---- */
            @if($errors->has('content'))
            editor.ui.view.element.classList.add('ck-editor--has-error');
            @endif

            /* ---- Sync editor content → textarea BEFORE form submits ---- */
