...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden max-w-2xl">
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
        ...
        <div class="space-y-4">
            ...
            <div>
                <label for="slug" class="block text-sm font-semibold text-slate-700 mb-1.5">Slug <span class="text-slate-400 font-normal">(Optional, auto-generated)</span></label>
                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full px-4 py-2.5 rounded-xl border {{ $errors->has('slug') ? 'border-red-300 focus:ring-red-500' : 'border-slate-300 focus:border-indigo-500 focus:ring-indigo-500/20' }} focus:ring-2 focus:outline-none transition-colors">
                @error('slug')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
