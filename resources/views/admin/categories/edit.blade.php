...
@section('page_subtitle', 'Update details for ' . $category->name)
    ...
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="p-6 sm:p-8 space-y-6" x-data="{
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border {{ $errors->has('description') ? 'border-red-300 focus:ring-red-500' : 'border-slate-300 focus:border-indigo-500 focus:ring-indigo-500/20' }} focus:ring-2 focus:outline-none transition-colors">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
